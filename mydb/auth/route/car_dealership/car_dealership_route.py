...
car_dealership_bp = Blueprint(
    "car_dealerships", __name__, url_prefix="/car_dealerships/"
)
...
def get_car_dealerships(car_dealership_id: int) -> Response:
    return make_response(
        jsonify(car_dealership_controller.find_by_id(car_dealership_id)), HTTPStatus.OK
    )
...
def create_car_dealership() -> Response:
    ...
    return make_response(
        f"CarDealership created with ID: {car_dealership_id}", HTTPStatus.CREATED
    )
...
def create_all_car_dealerships() -> Response:
    ...
    return make_response(
        car_dealership_controller.create_all(car_dealerships), HTTPStatus.CREATED
    )
